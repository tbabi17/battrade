<?
	class crm_db {
		...
		var $from_mail = "alert@battrade.mn";
	...
	class crm_corporate extends crm_retail {
		var $basic_fields = "crm_id,type,_class,regNo,firstName,lastName,engName,company_torol,employees,industry,industry_sub,phone,phone1,phone2,fax,email,www,_date,level,district,horoo,userCode,source,owner,mayDuplicate,customer_type,priority,pricetag,lat,lng,promo_code,promo_precent,promo_amount,descr,sorog_huchin";
	}
	...
	class crm_service extends crm_task{
		var $basic_fields = "service_id,crm_id,crm_name,subject,phone,service_revenue,service_debt,service_precent,closing_date,remind_date,service_stage,descr,owner,userCode,campaign,_date,pricetag,partner,right";
	}

	class crm_report_compare_product_user extends crm_task{
		var $basic_fields = "product_code,product_name,product_barcode,product_brand,amount,owner1,owner2,owner3,owner4,owner5,owner6,owner7,owner8,owner9,owner10";
	}
	...
	class crm_product_time_expired extends crm_task{
		var $basic_fields = "id,start_date,product_id,product_code,product_name,warehouse_id,position,end_date,warehouse_name";
	}
	...
	class crm_storage extends crm_task{
		var $basic_fields = "warehouse_id,product_id,product_code,warehouse_name,product_name,product_brand,aty,qty,pty,_date,unit_type";
	}
	
	class crm_product extends crm_task{
		var $basic_fields = "product_id,product_name,product_type,product_code,product_barcode,product_brand,product_vendor,price,unit_type,unit_size,company,price1,price2,price3,price4,price5,price6,price7,price8,price9,price10,warehouse_id,unit_metric,discount,userCode";
	}

	class crm_promotion extends crm_task{
		var $basic_fields = "id,promotion_name,start_date,end_date,summaryTotal,randomCount,promo_type,userCode,descr,discount,rank,p_type,promo_brand";
	}
	...
	class crm_user extends crm_task{
		var $basic_fields = "id,owner,password,fullName,phone,section,position,team,manager,gmailAccount,user_type,company,user_level,permission,partner,warehouse_id,mon,thue,wed,thur,fri,sat,sun,msg";
	}
	...
	class crm_goal extends crm_task{
		var $basic_fields = "id,owner,plan_name,team,product_id,product_name,start_date,end_date,price,count,amountTheshold,performCount,performTheshold,sku,_date,userCode";
	}
	...
	class crm_report_sales_precent extends crm_task{
		var $basic_fields = "deal_id,crm_id product_id,product_code,product_barcode,product_name, type,precent,warehouse_id,qty,pty,price,amount,_date,unit_size,flag,descr,crm_name";
	}
	...
	class crm_report_product extends crm_task{
		var $basic_fields = "product_id,product_barcode,product_code,product_brand,product_name,unit_size,qty,pty,amount,avg_price";
	}
	...
	class crm_report_storage_daily extends crm_task{
		var $basic_fields = "product_id,product_code,product_barcode,product_brand,product_name,unit_size,first,incoming,ret,sales,promo,last,bm1,bm2,shop,total,sku";
	}
	...
	class crm_report_sales_up_down extends crm_task{
		var $basic_fields = "monthName,amount";
	}
	...
	class crm {		
		...
		function crm_is_non_like($fls) {
			if ($fls == "id" || $fls == "_date" || $fls == "crm_id" || $fls == "mayDuplicate" || $fls == "parent_crm_id" || $fls == "customer_type" || $fls == "birthday" || $fls == "work_type" || $fls == "times" || $fls == "status" || $fls == "notify" || $fls == "deal_id" || $fls == "case_id" || $fls == "probablity" || $fls == "expected_revenue" || $fls == "actual_revenue" || $fls == "closing_date" || $fls == "qty" || $fls == "pty" || $fls == "total" || $fls == "partner" || $fls == "aty" || $fls == "product_id") 
				return true;
			...
			if (isset($query) && strlen($query) > 0) {
				...
				if (strlen($values) > 0) { 
					...
					for($i=0; $i < sizeof($fls); $i++) {
						...
						if ($fls[$i] == "product_name" || $fls[$i] == "product_code" || $fls[$i] == "product_brand") {
							$result=$result."product_id in (select product_id from crm_products where product_barcode like '%$query%' or product_code like '%$query%' or product_name like '%$query%' or product_brand like '%$query%') OR ";	
						} else
						if ($fls[$i] == "deal_name") {
		...
		function crm_query_list($item) {
			...
			while ($row = mysql_fetch_array($result)) {
				...
				if ($table == "crm_products") {
					...
					if ($fields == "unit_type")
						$record->put("value", $row['unit_type']);
					else
					if ($fields == "product_type")
						$record->put("value", $row['product_type']);
					else
					if ($fields == "product_brand")
						$record->put("value", $row['product_brand']);
					else
		...
		function crm_customer_xml_list($item) {
			...
			$wh = explode(',', $where);
			$owner = $wh[0];
			/*if($owner == 'bayarmaa1@battrade' || $owner == 'bayarmaa2@battrade')
				$owner = 'bayarmaa@battrade';*/
			$_date = $wh[1];			
			$q = "SELECT * FROM crm_customer WHERE owner='$owner'";
			if (strlen($_date) > 5) 
				$q = "SELECT * FROM crm_customer WHERE owner='$owner' and _date>=date(CURRENT_TIMESTAMP)";
			$record = new crm_customer();
			$rows = $this->query_result_rows("SELECT crm_id FROM crm_customer", " WHERE owner='$owner' ORDER BY $sort $dir");
		...
		function crm_test_xml_list($item) {
			...
			$where = $item->get("where");
			$wh = explode(',', $where);
			$q = "SELECT '654' as ddtd,'123' as qrdata,'no' as result,'' as lottery FROM crm_balance_by_padaan ";
			...
			//print_r($obj["qrData"]);
			if($qrdata != ""){
				$qins = "insert into crm_mta_integration (descr,billId,qrData,date,lottery,ttd,json) values ('$descr','$ddtd','$qrdata','$date','$lottery','$ttd','$jsondata')";
...
//				file_put_contents("c:\\123.txt", floor($datediff/(60*60*24)));
		...
		function crm_product_xml_list($item) {
			...
			$result = $this->query_result($q, " where length(product_type)>0 ORDER BY $sort $dir");
			while ($row = mysql_fetch_array($result)) {
		...
		function crm_product_available_xml_list($item) {
			//$query = "update crm_products as p set total = (select sum(incoming)-sum(outgoing)-ifnull((select sum(qty) from crm_deal_products where product_id=p.product_id and flag>0),0) from crm_balance as b where b.warehouse_id=1 and deposit='storage' and b.product_id=p.product_id) where length(product_type)>0 and total<=100";
			...
			$activity = $wh[2];
			$it = new $item();
/*			$it->put("values", "userCode");
			$it->put("where", $logged);
			$this->crm_service_products_to_back_action_by_one_crm($it);*/


			if($activity == "add_return"){
				$q = "SELECT product_id, 10000 as last FROM crm_products";
				$wheres = " ";
			}else{
				
				if($ware == 1){
					$q = "SELECT product_id, total as last FROM crm_products";
					$wheres = " WHERE 1=1 and warehouse_id=$ware and total>0";
				}else{
					$query = "delete from crm_deal_products_for_pavailable where userCode='$logged'";
					mysql_query($query, $this->db->conn());

					$query = "insert into crm_deal_products_for_pavailable (id,deal_id,service_id,product_id,precent,pty,qty,price,amount,type,crm_id,warehouse_id,_date,descr,userCode,unit_size,flag)
					(select id,deal_id,service_id,product_id,precent,pty,qty,price,amount,type,crm_id,warehouse_id,_date,descr,userCode,unit_size,flag from crm_deal_products where userCode='$logged' and _date>SUBDATE(now(), INTERVAL 5 day))";
					mysql_query($query, $this->db->conn());					

//					file_put_contents("c:\\ware.txt", $ware.' '.$logged.' '.$activity);
					$q = "SELECT product_id, sum(incoming)-sum(outgoing)-ifnull((select sum(qty) from crm_deal_products_for_pavailable as s  where s.warehouse_id =$ware and s.product_id=b.product_id and s.flag=1 limit 0,1),0) as last FROM crm_balance as b";
		//			$wheres = " WHERE 1=1 and deposit='storage' and warehouse_id=(select warehouse_id from crm_warehouse where owner='$logged' and warehouse_type='owner' limit 0,1) and _date>='2014-05-20' GROUP BY product_id";
					$wheres = " WHERE 1=1 and deposit='storage' and warehouse_id=$ware and _date>='2014-05-20' GROUP BY product_id having last>0";
//					file_put_contents("c:\\ware.txt", $ware.' '.$logged.' '.$activity);
				}
			}

			$result = $this->query_result($q, $wheres);
			...
			if($num_rows > 0){
				...
				$q = "SELECT 0 as product_id, 0 as last FROM crm_products";
				$wheres = " WHERE 1=1 limit 0,1";
				$result = $this->query_result($q, $wheres);
				while ($row = mysql_fetch_array($result)) {
					$record = new crm_product_available();
					$record->init($row);
		...
		function crm_warehouse_xml_list($item) {
			...
			$result = $this->query_result($q, " WHERE warehouse_type<>'container' and (warehouse_type='owner' and owner='$where' or warehouse_id=1) ORDER BY warehouse_id desc");
			while ($row = mysql_fetch_array($result)) {
		...
		function crm_service_list_xml_before() {
			...
			(select service_id,crm_id,subject,descr,phone,_date,closing_date,remind_date,owner,userCode,service_stage,service_revenue,service_precent,service_debt,service_qty,campaign,product_vendor,flag,warehouse_id,pricetag from crm_services where _date>SUBDATE(now(), INTERVAL 1 day))";
			mysql_query($query, $this->db->conn());
		
			$query_two = "select id from crm_deal_products  where service_id=0 and deal_id=0 and length(descr)>0";
			$result_two = mysql_query($query_two, $this->db->conn());
			while ($row_two = mysql_fetch_array($result_two)) {
					$id = $row_two['id'];
					$query = "update crm_deal_products set precent=(select discount from crm_products where product_id=crm_deal_products.product_id limit 0,1),service_id=(select service_id from crm_services_tmp where subject=crm_deal_products.descr and userCode=crm_deal_products.userCode and flag>0 and crm_id=crm_deal_products.crm_id limit 0,1) where id=$id";
					mysql_query($query, $this->db->conn());
			}
		}
		...
		function crm_service_xml_list($item) {
			...
			$sort = $item->get("sort");
			$dir = $item->get("dir");
			$crm_id = $item->get("where");
//			file_put_contents("c:\\last.txt", $crm_id);
		/*	$it = new $item();
			$it->put("values", "crm_id");
			$it->put("where", $crm_id);
			$this->crm_service_products_to_back_action_by_one_crm($it);*/
			...
			$result = $this->query_result($q, " WHERE crm_id=".$item->get("where")." ORDER BY _date DESC LIMIT 0,10");
			while ($row = mysql_fetch_array($result)) {
...
//			$where = $this->crm_session_userCode_where($where);
		...
		function crm_alarm_list($item) {
			...
			$q = "select 0 as crm_id,'dashboard' as type,'receipt' as status from crm_products_time_expired where  TIMESTAMPDIFF(DAY, end_date, NOW())  > -30 and TIMESTAMPDIFF(DAY, end_date, NOW()) <1";
			$count = $this->num_rows($q, "");
			$result = $this->query_result($q, " ");
			while ($row = mysql_fetch_array($result)) {
				$record = new crm_alarm();
		...
		function crm_warehouse_list($item) {
			...
			$where = $this->crm_where($item, $record->basic_fields);
			$count = $this->num_rows($q, $where);
			$result = $this->query_result($q, $where." ORDER BY warehouse_id  LIMIT $start,$end");
			while ($row = mysql_fetch_array($result)) {
				$record = new crm_warehouse();
				$record->init($row);
				$array[$count] = $record;
		...
		function crm_storage_list($item) {
			$query = "delete from crm_storage_list";

			$query1 = "delete from crm_deal_products_for_storage";
			mysql_query($query1, $this->db->conn());

			$query2 = "insert into crm_deal_products_for_storage (id,deal_id,service_id,product_id,precent,pty,qty,price,amount,type,crm_id,warehouse_id,_date,descr,userCode,unit_size,flag)
			(select id,deal_id,service_id,product_id,precent,pty,qty,price,amount,type,crm_id,warehouse_id,_date,descr,userCode,unit_size,flag from crm_deal_products where flag>0 and warehouse_id<>1)";
			mysql_query($query2, $this->db->conn());					
			...
			$json = "";
			$start = $item->get("start");
			$warehouse_id = $item->get("where");
			$end = $item->get("end");
			$sort = $item->get("sort");
			$dir = $item->get("dir");
			if($warehouse_id != 0){
				file_put_contents("c:\\asdf.txt", $warehouse_id);
			}else{
				$content = file_get_contents('c:\\asdf.txt', true);
				$warehouse_id = intval($content);
			}
//			if ($item->get("where") == 0) $item->put("where", "1");
			if($warehouse_id == 1){
				//$q = "SELECT '' as product_name,0 as qty,warehouse_id,product_id,ifnull((select total from crm_products where product_id=b.product_id and warehouse_id=$warehouse_id),0) as aty, sum(incoming)-sum(outgoing) as qty,sum(pty) as pty,max(_date) as _date FROM crm_balance as b";
				$q = "SELECT '' as product_name,0 as qty,warehouse_id,product_id,ifnull((select total from crm_products where product_id=b.product_id and warehouse_id=$warehouse_id),0) as aty, 0 as qty,sum(pty) as pty,max(_date) as _date FROM crm_balance as b";
			}else{
				$q = "SELECT '' as product_name,0 as qty,warehouse_id,product_id,ifnull((select total from crm_products where product_id=b.product_id and warehouse_id=$warehouse_id),0) as aty, sum(incoming)-sum(outgoing)-ifnull((select sum(qty) from crm_deal_products_for_storage as s  where s.warehouse_id =b.warehouse_id and s.product_id=b.product_id and s.flag=1 limit 0,1),0) as qty,sum(pty) as pty,max(_date) as _date FROM crm_balance as b";
			}

			$record = new crm_storage();
			//$where = $this->crm_where($item, $record->basic_fields);
			//$count = $this->num_rows($q, $where." and deposit='storage' and warehouse_id=1 and _date>='2014-05-20' GROUP BY product_id ORDER BY $sort $dir");
			$_SESSION['STORAGE_AVAILABLE_LIST'] = $q."WHERE 1=1 and deposit='storage' and warehouse_id=$warehouse_id and _date>='2014-05-20' GROUP BY product_id ";
			$result = $this->query_result($q, " WHERE 1=1 and deposit='storage' and warehouse_id=$warehouse_id and _date>='2014-05-20' GROUP BY product_id ");
//			echo $q." WHERE 1=1 and deposit='storage' and warehouse_id=$warehouse_id and _date>='2014-05-20' GROUP BY product_id ORDER BY $sort $dir";
			while ($row = mysql_fetch_array($result)) {
				$ph = explode('|', $this->crm_product_name_simple($row['product_id']));
				$row['product_name'] = $ph[0];
				$row['product_code'] = $ph[1];
				$row['product_brand'] = $ph[2];
				$row['warehouse_name'] = $this->crm_warehouse_name_simple($row['warehouse_id']);
				
				$a1 = $row['product_code'];
				$a2 = $row['product_name'];
				$a3 = $row['product_brand'];
				$a4 = $row['aty'];
				$a5 = $row['qty'];
				$a6 = $row['_date'];
				$a7 = $row['warehouse_name'];
				$a8 = $ph[5];

				$qins = "insert into crm_storage_list (product_code,product_name,avail_count,real_count,product_brand,warehouse_name,_date,unit_type) values ('$a1','$a2',$a4,$a5,'$a3','$a7','$a6','$a8')";
				$result_ins = mysql_query($qins, $this->db->conn());
			}
			...
			$json = "";
			$query = $item->get("query");
			$result = $this->query_result("select * from crm_storage_list where product_code like '%$query%' or product_name like '%$query%'", "ORDER BY $sort $dir");
			while ($row = mysql_fetch_array($result)) {
				$count++;
				$record = new crm_storage();
				$row['aty'] = $row['avail_count'];
				$row['qty'] = $row['real_count'];
				$record->init($row);
				$array[$count] = $record;
				$json .= $record->json().",";
			}
		...
		function crm_product_list($item) {
			...
			$record = new crm_product();
			$where = $this->crm_where($item, $record->basic_fields);
			$count = $this->num_rows($q, $where." and length(product_type)>0");
			$result = $this->query_result($q, $where." and length(product_type)>0 ORDER BY $sort $dir LIMIT $start,$end");
			$_SESSION['Product'] = $q.$where." and length(product_type)>0 ORDER BY $sort $dir";
			while ($row = mysql_fetch_array($result)) {
				$record = new crm_product();
				$record->init($row);
				$array[$count] = $record;
		...
		function crm_service_list_before() {							
			$content = file_get_contents('c:\\last_battrade.txt', true);
...
//			$result = $this->query_result("select max(id) as new_id from crm_deal_products", " ");
			...
			if ($last_id != $count_qr) {
				...
				$query_two = "select service_id,service_stage from crm_services where (service_stage='receipt' and _date>=date(CURRENT_TIMESTAMP)) or ((service_revenue=0 or service_revenue<service_debt) and service_stage<>'closed' and service_stage<>'fail' and _date>=SUBDATE(NOW(),3)) or ((service_revenue>0 or service_debt>0) and (service_stage='receipt' or service_stage='service') and _date>=SUBDATE(NOW(),3)) ";
					$result_two = mysql_query($query_two, $this->db->conn());
					while ($row_two = mysql_fetch_array($result_two)) {
							$service_id = $row_two['service_id'];
							$service_stage = $row_two['service_stage'];
							if($service_stage == "return"){
								$query1 = "UPDATE crm_services as b SET warehouse_id=(select warehouse_id from crm_warehouse where owner=b.userCode and warehouse_type='owner' and name not like '%хувиарт%' limit 0,1) where service_id=$service_id ";
								$result1 = mysql_query($query1, $this->db->conn());

								$query2 = "UPDATE crm_deal_products as b SET warehouse_id=(select warehouse_id from crm_warehouse where owner=b.userCode and warehouse_type='owner' and name not like '%хувиарт%' limit 0,1) where service_id=$service_id and _date>=SUBDATE(NOW(),3)";
								$result2 = mysql_query($query2, $this->db->conn());
							}
				...
				file_put_contents("c:\\last_battrade.txt", $count_qr);
			}
			...
			if ($a || $b || ($u_service_product_id == 0 || $u_service_id == 0)) {
				...
				if ($a && $u_service_id > 0) {
					$query = "UPDATE crm_services as b SET service_revenue=(select sum(amount) from crm_deal_products as c where c.service_id=b.service_id),service_debt=(select sum(amount) from crm_deal_products as c where c.service_id=b.service_id and type='loan')-ifnull((select sum(amount) from crm_service_payroll as d where d.service_id=b.service_id),0)-service_revenue*service_precent/100 where service_stage<>'closed' and (service_revenue=0 or service_debt=0 or (service_stage='receipt' and service_id>=$u_service_id)) and _date>=DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 3 day)";
					$result = mysql_query($query, $this->db->conn());
				}

				if (!$a && $b && strlen($descr) > 0) {
					$query = "UPDATE crm_services as b SET service_revenue=(select sum(amount) from crm_deal_products as c where c.service_id=b.service_id),service_debt=(select sum(amount) from crm_deal_products as c where c.service_id=b.service_id and type='loan')-ifnull((select sum(amount) from crm_service_payroll as d where d.service_id=b.service_id),0)-service_revenue*service_precent/100 where service_stage<>'closed' and (service_revenue=0 or service_debt=0 or (service_stage='receipt' and subject='$descr')) and _date>=DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 3 day)";
					$result = mysql_query($query, $this->db->conn());
				}
			}
			
			$query = "UPDATE crm_services as b SET service_debt=service_revenue where service_debt=0 and service_revenue>0";
			$result = mysql_query($query, $this->db->conn());
			
			$query = "UPDATE crm_services as b SET service_revenue=(select sum(amount) from crm_deal_products as c where c.service_id=b.service_id),service_debt=(select sum(amount) from crm_deal_products as c where c.service_id=b.service_id and type='loan')-ifnull((select sum(amount) from crm_service_payroll as d where d.service_id=b.service_id),0)-service_revenue*service_precent/100 where service_stage='receipt' and service_revenue=0 and flag>0 and _date>=date(CURRENT_TIMESTAMP)";
			$result = mysql_query($query, $this->db->conn());
			*/
		}

		function crm_balance_minid_insert() {
			$content = file_get_contents('c:\\min.txt', true);
			$last_id = intval($content);
			$result = $this->query_result("SELECT MIN(id) as new_id FROM crm_balance WHERE DATE( _date ) = DATE( DATE_ADD( CURRENT_TIMESTAMP , INTERVAL -1 DAY ) )", " ");
			while ($row = mysql_fetch_array($result)) {
				if ($last_id < intval($row['new_id'])) {
					file_put_contents("c:\\min.txt", $row['new_id']);
				}
			}			
		}
		...
		function crm_service_list($item) {
			...
			if (strlen($values) == 0) {
				...
				$this->crm_balance_minid_insert();
				$this->crm_insert_price_history();
				//$this->crm_service_products_to_sales_action($item);			
			}		
			$array = array();
			$count = 0;
			...
			$dir = "asc";
			$date_where = $this->date_where($item);
			$q = "SELECT *,0 as total,(select partner from crm_users where owner=b.userCode) as partner FROM crm_services_by_customer_report as b";
...
//				$where .= " and (warehouse_id=$warehouse_id or userCode='$logged')";
				$where .= " and (1=1 or userCode='$logged')";
			
			$spec_where = "";
			if ($views == "Incoming Services")
				$spec_where = " or (service_stage='receipt')";

			$logged = $_SESSION['logged'];
			$owner = $logged['owner'];
			//$count = $this->num_rows($q, $where." $date_where $spec_where and (service_revenue>0 or service_stage='return') ORDER BY $sort $dir");	
			$count = 0;
			$result = $this->query_result($q, $where." $date_where $spec_where and (service_revenue>0 or service_stage='return') ORDER BY $sort $dir ");
			$_SESSION['Service'] = $q." ".$where." ORDER BY $sort $dir";			
			while ($row = mysql_fetch_array($result)) {
				if (intval($row['service_revenue']) > 0 || $row['service_stage'] == 'return') {
					$record = new crm_service();
					$row['crm_name'] = $this->crm_customer_name($row['crm_id']);
					if($row['service_stage'] == 'service'){
						$row['right'] = $this->crm_sub_calculate_amount($row['subject'],$row['service_revenue']);
					}
					//$row['notify'] = $this->crm_service_post_notify($row['service_id']);
					$count++;
					$record->init($row);
					$array[$count] = $record;
					$json .= $record->json().",";
		...
		function crm_insert_price_history(){
			$count_bt = $this->num_rows("SELECT * FROM crm_products_price_history WHERE DATE(_date)=DATE(CURRENT_TIMESTAMP)", "");

			if($count_bt <= 0){
				$query = "INSERT INTO crm_products_price_history (product_id,product_name,product_type,product_code,product_barcode,product_brand,product_vendor,unit_type,unit_size,unit_metric,price,company,_date,price1,price2,price3,price4,price5,price6,price7,price8,price9,price10,userCode,warehouse_id,total,unit_res,discount,_class) select product_id,product_name,product_type,product_code,product_barcode,product_brand,product_vendor,unit_type,unit_size,unit_metric,price,company,CURRENT_TIMESTAMP,price1,price2,price3,price4,price5,price6,price7,price8,price9,price10,userCode,warehouse_id,total,unit_res,discount,_class from crm_products";
				mysql_query($query, $this->db->conn());
			}			
		}
		...
		function crm_deal_product_list($item) {
			...
			$where = $this->crm_where($item, $record->basic_fields);
			$count = $this->num_rows($q, $where);
			$result = $this->query_result($q, $where." ORDER BY (select product_code from crm_products as p where p.product_id=crm_deal_products.product_id) LIMIT $start,$end");
			while ($row = mysql_fetch_array($result)) {
				$record = new crm_deal_product();
				$v = $this->crm_product_name_simple($row['product_id']);
				$vw = explode('|', $v);
		...
		function crm_competitor_list($item) {
			...
			$result = $this->query_result("(select sum(incoming) as q,sum(amount) as a from crm_balance where user_code='$owner' and _date between '$start_date' and '$end_date' and deposit='sales' and crm_id not in (select crm_id from crm_customer where _class='AGENT'))", "");
			$return = "0,0";
			while ($row = mysql_fetch_array($result)) {
				$return = $row['q'].",".$row['a'];
			}
		...
		function crm_user_planning_list($item) {
			...
			$where = $this->crm_session_where($where);
			$where .= $this->crm_view_query($item);
			$count = $this->num_rows($q, $where." and CURRENT_TIMESTAMP between start_date and end_date ORDER BY _class $dir");
			$result = $this->query_result($q, $where." and CURRENT_TIMESTAMP between start_date and end_date ORDER BY _class $dir LIMIT $start,$end");
			while ($row = mysql_fetch_array($result)) {
				$record = new crm_goal();
...
//				$row['performTheshold'] = 0;
				$count = $row['amountTheshold'];
				if ($count == 0) $count = 1;
				if ($row['performTheshold'] >= $row['amountTheshold'])
					$row['sku'] = 100;
				else
					$row['sku'] = $row['performTheshold'] * 100 / $count;
		...
		function crm_owner_orson_point_info($owner, $item) {	
			...
			$end_date = $item->get("end_date");
			$count = 0;
			$result = $this->query_result("(SELECT SUM( points ) as count FROM (SELECT COUNT( DISTINCT crm_id ) AS points FROM crm_services_by_customer_report WHERE _date BETWEEN  '$start_date' AND  '$end_date' AND userCode = '$owner' GROUP BY DATE( _date )) AS t1)", "");
			while ($row = mysql_fetch_array($result)) {
				$count += $row['count'];
			}
		...
		function crm_owner_hiisen_point_info($owner, $item) {	
			...
			$end_date = $item->get("end_date");
			$count = 0;
			$result = $this->query_result("(SELECT SUM( points ) as count FROM (SELECT COUNT( DISTINCT crm_id ) AS points FROM crm_balance_by_customer_report WHERE _date BETWEEN  '$start_date' AND  '$end_date' AND user_code = '$owner' GROUP BY DATE( _date )) AS t1)", "");
			while ($row = mysql_fetch_array($result)) {
				$count += $row['count'];
			}
		...
		function crm_owner_cash_total_info($owner, $item) {	
			...
			$end_date = $item->get("end_date");
			$count = 0;
			$result = $this->query_result("select sum(amount) as amount  from crm_balance_by_customer_report where user_code='$owner' and _date between '$start_date' and '$end_date' and deposit='cash' and product_id>0 and crm_id not in (select crm_id from crm_customer where _class='AGENT')", "");
			while ($row = mysql_fetch_array($result)) {
				$count += $row['amount'];
			}
		...
		function crm_owner_loan_total_info($owner, $item) {	
			$start_date = $item->get("start_date");
			$end_date = $item->get("end_date");
			$count = 0;
			$result = $this->query_result("select sum(amount) as amount  from crm_balance_by_customer_report where user_code='$owner' and _date between '$start_date' and '$end_date' and deposit='lease' and product_id>0 and crm_id not in (select crm_id from crm_customer where _class='AGENT')", "");
			while ($row = mysql_fetch_array($result)) {
		...
		function crm_owner_sum_total_info($owner, $item) {	
			...
			$end_date = $item->get("end_date");
			$count = 0;
			$result = $this->query_result("select sum(amount) as amount from crm_balance_by_customer_report where user_code='$owner' and _date between '$start_date' and '$end_date' and (deposit='sales' or (product_id=0 and deposit='cash')) and crm_id not in (select crm_id from crm_customer where _class='AGENT')", "");
			while ($row = mysql_fetch_array($result)) {
				$count += abs($row['amount']);
			}
		...
		function crm_report_user_list($item) {
			...
			$sort = $item->get("sort");
			$dir = $item->get("dir");
			$q = "SELECT owner,0 as car_uld from crm_users as u";
		...
		function crm_report_sales_precent_list($item) {
			$array = array();
			$count = 0;
			$json = "";
			$start = $item->get("start");
			...
			$query = "delete from crm_deal_products_for_sales_report";
			mysql_query($query, $this->db->conn());

			$query = "insert into crm_deal_products_for_sales_report (id,deal_id,service_id,product_id,precent,pty,qty,price,amount,type,crm_id,warehouse_id,_date,descr,userCode,unit_size,flag)
			(select id,deal_id,service_id,product_id,precent,pty,qty,price,amount,type,crm_id,warehouse_id,_date,descr,userCode,unit_size,flag from crm_deal_products where _date between '$start_date' and '$end_date' and userCode='$owner')";
			mysql_query($query, $this->db->conn());					

			$logged = $_SESSION['logged']['owner'];
			$sort = $item->get("sort");
			$dir = $item->get("dir");
			$q = "SELECT deal_id,service_id,product_id,precent,pty,qty,price,amount,type,crm_id,warehouse_id,_date,descr,userCode,unit_size,flag from crm_deal_products_for_sales_report";
			$record = new crm_report_user();
			$where = " WHERE 1=1 and userCode='$owner'";

			$_SESSION['CRM_REPORT_SALES_PRECENT'] = "SELECT deal_id,service_id,product_id,precent,pty,qty,price,amount,type,crm_id,warehouse_id,_date,descr,userCode,unit_size,flag from crm_deal_products_for_sales_report  WHERE 1=1 and userCode='$owner'";
			$result = $this->query_result($q, $where);
			while ($row = mysql_fetch_array($result)) {
				$v = $this->crm_product_name_simple($row['product_id']);
				$vw = explode('|', $v);
				...
				$row['unit_size'] = $vw[3];
				$row['product_barcode'] = $vw[4];
				$record = new crm_report_sales_precent();
				$record->init($row);
				$array[$count] = $record;
				$json .= $record->json().",";
			}
		...
		function crm_prepare_sub_bal_ser_for_report($start_date, $end_date) {
			$qins = "delete from crm_services_by_customer_report";
			mysql_query($qins, $this->db->conn());
			$qry = "insert into crm_services_by_customer_report (service_id,crm_id,subject,descr,phone,_date,closing_date,remind_date,owner,userCode,service_stage,service_revenue,service_precent,service_debt,service_qty,campaign,product_vendor,flag,warehouse_id,pricetag)
			(select service_id,crm_id,subject,descr,phone,_date,closing_date,remind_date,owner,userCode,service_stage,service_revenue,service_precent,service_debt,service_qty,campaign,product_vendor,flag,warehouse_id,pricetag from crm_services where _date between '$start_date' and '$end_date')";
			...
			(id,incoming,outgoing,pty,type,price,product_id,crm_id,warehouse_id,owner,_date,amount,descr,deposit,user_code)
			 select id,incoming,outgoing,pty,type,price,product_id,crm_id,warehouse_id,owner,_date,amount,descr,deposit,user_code from crm_balance where _date between '$start_date' and '$end_date'";
			mysql_query($qry, $this->db->conn());				
		}
		...
		function crm_report_product_list($item) {
			...
			$dir = $item->get("dir");
			if($type == 'ирсэн'){
				$q = "SELECT product_id,sum(qty) as qty,sum(pty) as pty,sum(amount) as amount, avg(price) as avg_price from crm_deal_products";
				$result = $this->query_result($q, "WHERE _date between '$start_date' and '$end_date' and flag=1 group by product_id ORDER BY product_id ASC");
			}else{
				if($type == 'бүгд'){
					$q = "SELECT product_id,sum(qty) as qty,sum(pty) as pty,sum(amount) as amount, avg(price) as avg_price from crm_deal_products";
					$result = $this->query_result($q, "WHERE _date between '$start_date' and '$end_date' group by product_id ORDER BY product_id ASC");
				}else{
					$q = "SELECT product_id,sum(incoming) as qty,sum(pty) as pty,sum(amount) as amount, avg(price) as avg_price from crm_balance";
					$result = $this->query_result($q, "WHERE _date between '$start_date' and '$end_date' and deposit='sales' group by product_id ORDER BY product_id ASC");
				}
			...
			while ($row = mysql_fetch_array($result)) {
				...
				$a9 = doubleval($row['avg_price']);$a10 = doubleval($row['product_id']);
				$qins = "insert into crm_report_product (product_code,product_barcode,product_brand,product_name,unit_size,qty,pty,amount,avg_price,product_id) values ('$a1','$a2','$a3','$a4',$a5,$a6,$a7,$a8,$a9,$a10)";
				$result_ins = mysql_query($qins, $this->db->conn());
			}
			$count = 0;
			$query = $item->get("query");
			if (strlen($query) > 0) 
				$where = "where (product_name like '%$query%' or product_brand like '%$query%' or product_code like '%$query%' or product_barcode like '%$query%')";
			else
				$where = "";
			$result = $this->query_result("SELECT * FROM crm_report_product", "$where ORDER BY $sort $dir");
			$_SESSION['CRM_REPORT_PRODUCT'] = "SELECT * FROM crm_report_product $where ORDER BY $sort $dir";
			while ($row = mysql_fetch_array($result)) {
		...
		function crm_report_customer_list($item) {
			...
			$sort = $item->get("sort");
			$dir = $item->get("dir");
			$q = "select crm_id, SUM(IF(deposit = 'lease' and _date<'$start_date', amount,0)) as first, SUM(IF(deposit = 'cash', amount,0)) as paid, SUM(IF(deposit = 'back', amount,0)) as ret, SUM(IF(deposit = 'sales', amount,0)) as amount, SUM(IF(deposit = 'sales' and warehouse_id=1, amount,0)) as amount1, SUM(IF(deposit = 'sales' and warehouse_id=2, amount,0)) as amount2, SUM(IF(deposit = 'discount', amount,0)) as discount, SUM(IF(deposit = 'changeprice', amount,0)) as changeprice from crm_balance_by_customer_report";
			$wh = $item->get("where");
			$wh1 = explode(',', $wh);
			$owner = $wh1[0];
			$result = $this->query_result($q, "WHERE user_code='$owner' and _date between '$start_date' and '$end_date' and type<>7 group by crm_id ORDER BY $sort $dir");
			$_SESSION['CRM_REPORT_CUSTOMER'] = $q." WHERE user_code='$owner' and _date between '$start_date' and '$end_date' and type<>7 group by crm_id ORDER BY $sort $dir";
			while ($row = mysql_fetch_array($result)) {
				$row['crm_name'] = $this->crm_customer_name_simple($row['crm_id']);
			...
			$query = $item->get("query");
			if (strlen($query) > 0) 
				$where = "where (crm_name like '%$query%')";
			else
				$where = "";
			$result = $this->query_result("SELECT crm_name,old_balance as first,huns as amount1,goo as amount2,total as amount,paid,ret,changeprice,discount,new_balance as last FROM crm_report_customer", "$where ORDER BY $sort $dir");
			$_SESSION['CRM_REPORT_CUSTOMER'] = "SELECT crm_name,oldbalance as first,amount1,amount2,amount,paid,ret,changeprice,discount,newbalance as last FROM crm_report_customer $where ORDER BY $sort $dir";
			while ($row = mysql_fetch_array($result)) {
		...
		function crm_report_compare_customer_list($item) {
			...
			$sort = $item->get("sort");
			$dir = $item->get("dir");
			$q = "SELECT user_code as owner,crm_id,SUM(IF(deposit = 'sales' and month(_date)=1, amount,0)) as month1,SUM(IF(deposit = 'sales' and month(_date)=2, amount,0)) as month2,SUM(IF(deposit = 'sales' and month(_date)=3, amount,0)) as month3,SUM(IF(deposit = 'sales' and month(_date)=4, amount,0)) as month4,SUM(IF(deposit = 'sales' and month(_date)=5, amount,0)) as month5,SUM(IF(deposit = 'sales' and month(_date)=6, amount,0)) as month6,SUM(IF(deposit = 'sales' and month(_date)=7, amount,0)) as month7,SUM(IF(deposit = 'sales' and month(_date)=8, amount,0)) as month8,SUM(IF(deposit = 'sales' and month(_date)=9, amount,0)) as month9,SUM(IF(deposit = 'sales' and month(_date)=10, amount,0)) as month10,SUM(IF(deposit = 'sales' and month(_date)=11, amount,0)) as month11,SUM(IF(deposit = 'sales' and month(_date)=12, amount,0)) as month12 from crm_balance_for_compare_report";
			...
			if($owner != ""){
				$qins = "delete from crm_balance_for_compare_report";
				mysql_query($qins, $this->db->conn());
				$qry = "insert into crm_balance_for_compare_report 
				(id,incoming,outgoing,pty,type,price,product_id,crm_id,warehouse_id,owner,_date,amount,descr,deposit,user_code)
				 select id,incoming,outgoing,pty,type,price,product_id,crm_id,warehouse_id,owner,_date,amount,descr,deposit,user_code from crm_balance where YEAR(_date)=YEAR(CURRENT_TIMESTAMP) and deposit='sales'";
				mysql_query($qry, $this->db->conn());				

				$qry1 = "insert into crm_balance_for_compare_report 
				(id,incoming,outgoing,pty,type,price,product_id,crm_id,warehouse_id,owner,_date,amount,descr,deposit,user_code)
				 select id,incoming,outgoing,pty,type,price,product_id,crm_id,warehouse_id,owner,_date,amount,descr,deposit,user_code from crm_balance_old1 where YEAR(_date)=YEAR(CURRENT_TIMESTAMP) and deposit='sales'";
				mysql_query($qry1, $this->db->conn());		
			}

			$product_id = $wh[1];
			$swhere = "WHERE user_code='$owner' and crm_id not in (select crm_id from crm_customer where _class='AGENT') GROUP BY user_code,crm_id ORDER BY owner,crm_id asc";
		...
		function crm_report_compare_product_user_list($item) {
			$qins = "delete from crm_report_compare_product_user";
			mysql_query($qins, $this->db->conn());
			$array = array();
			$count = 0;
			$json = "";
			...
			$sort = $item->get("sort");
			$dir = $item->get("dir");
			$q = "select product_id,SUM(IF(user_code = 'bayarjargal@battrade', ".$field.",0)) as owner1,SUM(IF(user_code = 'batchuluun@battrade', ".$field.",0)) as owner2,SUM(IF(user_code = 'bayarmaa@battrade', ".$field.",0)) as owner3,SUM(IF(user_code = 'dorjsuren@battrade', ".$field.",0)) as owner4,SUM(IF(user_code = 'munkhtulga@battrade', ".$field.",0)) as owner5,SUM(IF(user_code = 'lodonsharav@battrade', ".$field.",0)) as owner6,SUM(IF(user_code = 'battulga@battrade', ".$field.",0)) as owner7,SUM(IF(user_code = 'dorjsuren@battrade', ".$field.",0)) as owner8,SUM(IF(user_code = 'nyamdorj@battrade', ".$field.",0)) as owner9,SUM(IF(user_code = 'otgonchimeg@cosmo', ".$field.",0)) as owner10 from crm_balance where deposit='sales' and crm_id not in (select crm_id from crm_customer where _class='AGENT') and _date between '$start_date' and '$end_date' group by product_id";
			...
			while ($row = mysql_fetch_array($result)) {
				...
				$owner9 = $row['owner9'];
				$owner10 = $row['owner10'];
				$amount = $row['owner1']+$row['owner2']+$row['owner3']+$row['owner4']+$row['owner5']+$row['owner6']+$row['owner7']+$row['owner8']+$row['owner9']+$row['owner10'];
				
				$qins = "insert into crm_report_compare_product_user (product_id,product_code,product_name,product_barcode,product_brand,amount,owner1,owner2,owner3,owner4,owner5,owner6,owner7,owner8,owner9,owner10) values ('$product_id',$product_code,'$product_name','$product_barcode','$product_brand',$amount,$owner1,$owner2,$owner3,$owner4,$owner5,$owner6,$owner7,$owner8,$owner9,$owner10)";
				
				mysql_query($qins, $this->db->conn());
			}					
			
			$query = $item->get("query");			
			$q = "select * from crm_report_compare_product_user";
			if (strlen($query) > 0)
				$q = "select * from crm_report_compare_product_user where product_code like '%$query%' or product_name like '%$query%' or product_brand like '%$query%' or product_barcode like '%$query%'";
			$result = $this->query_result($q, " ORDER BY $sort $dir");

			$_SESSION['CRM_REPORT_COMPARE'] = $q." ORDER BY $sort $dir";
			while ($row = mysql_fetch_array($result)) {
				$record = new crm_report_compare_product_user();				
				$record->init($row);
				$array[$count] = $record;
				$json .= $record->json().",";
			}
		...
		function crm_report_compare_user_list($item) {
			...
			$sort = $item->get("sort");
			$dir = $item->get("dir");
			$q = "select user_code as owner,SUM(IF(deposit = 'sales' and month(_date)=1, amount,0)) as month1,SUM(IF(deposit = 'sales' and month(_date)=2, amount,0)) as month2,SUM(IF(deposit = 'sales' and month(_date)=3, amount,0)) as month3,SUM(IF(deposit = 'sales' and month(_date)=4, amount,0)) as month4,SUM(IF(deposit = 'sales' and month(_date)=5, amount,0)) as month5,SUM(IF(deposit = 'sales' and month(_date)=6, amount,0)) as month6,SUM(IF(deposit = 'sales' and month(_date)=7, amount,0)) as month7,SUM(IF(deposit = 'sales' and month(_date)=8, amount,0)) as month8,SUM(IF(deposit = 'sales' and month(_date)=9, amount,0)) as month9,SUM(IF(deposit = 'sales' and month(_date)=10, amount,0)) as month10,SUM(IF(deposit = 'sales' and month(_date)=11, amount,0)) as month11,SUM(IF(deposit = 'sales' and month(_date)=12, amount,0)) as month12 from crm_balance_for_compare_report where deposit='sales' and user_code like '%@%' and crm_id not in (select crm_id from crm_customer where _class='AGENT') group by user_code";
			$result = $this->query_result($q, "");
			while ($row = mysql_fetch_array($result)) {
				$owner = $row['owner'];
				$month1 = $row['month1'];
		...
		function crm_report_compare_product_list($item) {
			...
			$q = "select product_id,SUM(IF(deposit = 'sales' and month(_date)=1, ".$field.",0)) as month1,SUM(IF(deposit = 'sales' and month(_date)=2, ".$field.",0)) as month2,SUM(IF(deposit = 'sales' and month(_date)=3, ".$field.",0)) as month3,SUM(IF(deposit = 'sales' and month(_date)=4, ".$field.",0)) as month4,SUM(IF(deposit = 'sales' and month(_date)=5, ".$field.",0)) as month5,SUM(IF(deposit = 'sales' and month(_date)=6, ".$field.",0)) as month6,SUM(IF(deposit = 'sales' and month(_date)=7, ".$field.",0)) as month7,SUM(IF(deposit = 'sales' and month(_date)=8, ".$field.",0)) as month8,SUM(IF(deposit = 'sales' and month(_date)=9, ".$field.",0)) as month9,SUM(IF(deposit = 'sales' and month(_date)=10, ".$field.",0)) as month10,SUM(IF(deposit = 'sales' and month(_date)=11, ".$field.",0)) as month11,SUM(IF(deposit = 'sales' and month(_date)=12, ".$field.",0)) as month12 from crm_balance_for_compare_report where _date>'2014-06-01' and deposit='sales' and crm_id not in (select crm_id from crm_customer where _class='AGENT') group by product_id";
		...
		function crm_report_storage_daily_list($item) {
			...
			SUM(IF(deposit='storage' and warehouse_id=5 and amount=0 and _date<'$end_date', incoming, 0)) as first,
			0 as incoming,			
			SUM(IF(deposit='storage' and  type=4 and _date between '$start_date' and '$end_date', incoming, 0)) as ret,
			SUM(IF(deposit='storage' and _date between '$start_date' and '$end_date' and price>0 and warehouse_id=(select warehouse_id from crm_warehouse where owner='$userCode' and warehouse_type='owner' limit 0,1 ), outgoing, 0)) as sales,
			SUM(IF(deposit='sales' and _date between '$start_date' and '$end_date' and price=0 and user_code='$userCode', incoming, 0)) as promo,
			0 as bm1,
			0 as sku,
			0 as bm2,
			0 as bm3,
			...
			$where = $this->crm_where($item, $record->basic_fields);
			$where = $this->crm_session_where($where);
			$count = $this->num_rows($q, $where." and  warehouse_id=(select warehouse_id from crm_warehouse where owner='$userCode' and warehouse_type='owner' limit 0,1 ) group by product_id ORDER BY warehouse_id,product_id ASC");			
			$result = $this->query_result($q, $where." and warehouse_id=(select warehouse_id from crm_warehouse where owner='$userCode' and warehouse_type='owner' limit 0,1 ) group by product_id  ORDER BY warehouse_id,product_id ASC");
			$_SESSION['CRM_REPORT_STORAGE_DAILY'] = $q." ".$where." and warehouse_id=(select warehouse_id from crm_warehouse where owner='$userCode' and warehouse_type='owner' limit 0,1 ) group by product_id  ORDER BY warehouse_id,product_id ASC";
		//	echo $q, $where." and product_id > 0 and warehouse_id=(select warehouse_id from crm_warehouse where owner='$userCode' and warehouse_type='owner' limit 0,1 ) group by product_id  ORDER BY warehouse_id,product_id ASC";
			while ($row = mysql_fetch_array($result)) {
				...
				$row['unit_size'] = $vw[3];
				$row['product_barcode'] = $vw[4];
				if( doubleval($row['sales']) > 0 && doubleval($row['first'])>0){
					$row['sku'] = doubleval($row['sales'])*100/doubleval($row['first']);
				}
		...
		function crm_report_storage_list($item) {
			...
			$dir = $item->get("dir");
			$q = "SELECT product_id,'' as product_brand,
			SUM(IF(deposit='storage' and _date between '2014-05-20' and '$start_date', incoming-outgoing, 0)) as first,
			...
			$where = $this->crm_where($item, $record->basic_fields);
			$where = $this->crm_session_where($where);
			$count = $this->num_rows($q, $where." and product_id > 0 and warehouse_id<3 group by product_id ORDER BY warehouse_id,product_id ASC");			
			$result = $this->query_result($q, $where." and product_id > 0 and warehouse_id<3 group by product_id ORDER BY warehouse_id,product_id ASC");
			$_SESSION['CRM_REPORT_STORAGE'] = $q." ".$where." and product_id > 0 and warehouse_id<3 group by product_id  ORDER BY product_id ASC";
			while ($row = mysql_fetch_array($result)) {
				$record = new crm_report_storage();
				$v = $this->crm_product_name_simple($row['product_id']);
				$vw = explode('|', $v);
...
//			$where = $this->crm_session_where($where);
		...
		function crm_chart_product_brand_list($item) {
			...
			$field = "amount";
			if ($wh[1] == 2) $field = "pty";
			$q = "SELECT product_brand, SUM(".$field.") as amount FROM crm_balance_by_customer_report AS b JOIN crm_products AS p ON b.product_id = p.product_id";
		...
		function crm_chart_sales_up_down_list($item) {
			...
			$fds = "allz";
			$wh = " WHERE deposit='sales' and year(_DATE)=year(CURRENT_TIMESTAMP) group by month(_date)";
			if ($values == "byowner") {
				$q = "SELECT month(_date) as monthName,userCode,sum(".$field.") as amount from crm_balance";
				$wh = " WHERE deposit='sales' and locate(userCode,'$filter')>0 group by month(_date),userCode";
				$fds = 'userCode';
			}
			if ($values == "byproduct") {
		...
		function crm_owner_point_list($owner, $date) {
			...
			$q = "SELECT count(crm_id) as c FROM crm_customer WHERE owner='$owner' and descr=(select $wd from crm_users where owner='$owner' limit 0,1)";
			...
			$q = "SELECT count(distinct crm_id) as c FROM crm_services_for_gps WHERE date(_date)=date('$date') and userCode='$owner'";
...
//			$where = $this->crm_session_where($where);
		...
		function crm_user_gps($owner) {
			$q = "select lat,lng,_date from crm_gps_for_map where owner='$owner' order by _date desc limit 0,1";
			$result = $this->query_result($q, "");
			while ($row = mysql_fetch_array($result)) {
				return $row;
			}
		...
		function crm_product_time_expired_list($item) {
			$array = array();
			$count = 0;
			$json = "";
			$start = $item->get("start");
			...
			$sort = $item->get("sort");
			$dir = $item->get("dir");
			$q = "SELECT id,start_date,product_id,warehouse_id,position,end_date from crm_products_time_expired";
 
			$where = $this->crm_session_where($where);
			$count = 5;
			$result = $this->query_result($q, $where." WHERE 1=1 and TIMESTAMPDIFF(DAY, end_date, NOW()) <1 ");
			while ($row = mysql_fetch_array($result)) {
				$record = new crm_product_time_expired();
				$v = $this->crm_product_name_simple($row['product_id']);
				$vw = explode('|', $v);
				$row['product_name'] = $vw[0];
				$row['product_code'] = $vw[1];
		...
		function crm_chart_gps_last_list($item) {
			...
			$query = "delete from crm_services_for_gps";
			mysql_query($query, $this->db->conn());

			$query = "insert into crm_services_for_gps (service_id,crm_id,subject,descr,phone,_date,closing_date,remind_date,owner,userCode,service_stage,service_revenue,service_precent,service_debt,service_qty,campaign,product_vendor,flag,warehouse_id,pricetag)
			(select service_id,crm_id,subject,descr,phone,_date,closing_date,remind_date,owner,userCode,service_stage,service_revenue,service_precent,service_debt,service_qty,campaign,product_vendor,flag,warehouse_id,pricetag from crm_services where DATE(_date)=DATE(CURRENT_TIMESTAMP))";
			mysql_query($query, $this->db->conn());

			$query = "delete from crm_gps_for_map";
			mysql_query($query, $this->db->conn());

			$query = "insert into crm_gps_for_map (id,owner,lat,lng,_date,crm_id,battery)
			(select id,owner,lat,lng,_date,crm_id,battery from crm_gps where DATE(_date)=DATE(CURRENT_TIMESTAMP))";
			mysql_query($query, $this->db->conn());
			...
			$sort = $item->get("sort");
			$dir = $item->get("dir");
			$q = "SELECT id,owner,0 as lat,0 as lng,'' as _date from crm_users as c";
			$record = new crm_gps();
//			$where = "";
//			$where = $this->crm_where($item, $record->basic_fields);
			$where = $this->crm_session_where($where);
			$count = 5;//$this->num_rows($q, $where." WHERE position='Борлуулагч' and (_date>=date(CURRENT_TIMESTAMP) or owner in (select owner from crm_gps where _date>=date(CURRENT_TIMESTAMP) group by owner))");
//			$result = $this->query_result($q, $where." WHERE position='Борлуулагч' and owner in (select owner from crm_gps where _date>=date(CURRENT_TIMESTAMP) group by owner)");
			$result = $this->query_result($q, $where." WHERE 1=1 and user_level=0 group by owner");
			while ($row = mysql_fetch_array($result)) {
				$record = new crm_gps();
				$pow = $this->crm_owner_point_list($row['owner'], $start_date);
				$gps = $this->crm_user_gps($row['owner']);
				$row['lat'] = $gps['lat'];
				$row['lng'] = $gps['lng'];
				$row['_date'] = $gps['_date'];
				$row['must'] = $pow['must'];
...
//			$where = $this->crm_where($item, $record->basic_fields);
		...
		function crm_sub_calculate_amount($subject,$service_revenue) {
			$value = 'no';
			$content = file_get_contents('c:\\min.txt', true);
			$last_id = intval($content);
			$result = $this->query_result("SELECT sum(amount) as amount FROM crm_balance_by_customer_report", " WHERE descr=$subject and deposit='sales' and id>$last_id");
			while ($row = mysql_fetch_array($result)) {
				if($row['amount'] == $service_revenue)
					$value = 'yes';
			}
		...
		function crm_service_post_notify($service_id) {
			$logged = $_SESSION["logged"]['owner'];
			$result_one = $this->query_result("SELECT * FROM crm_posts", " WHERE service_id=$service_id and length(reply_id)=0 and owner='$logged'");
			$rows = mysql_num_rows($result_one);
			if ($rows > 0)
				return '<div class="noti_bubble_small">'.$rows.'</div>';
		...
		function crm_product_name_simple($product_id) {
			$result = $this->query_result("SELECT product_name,product_code,product_brand,unit_size,product_barcode,unit_type FROM crm_products", " WHERE product_id=$product_id");
			while ($row = mysql_fetch_array($result)) {			
				return $row['product_name'].'|'.$row['product_code'].'|'.$row['product_brand'].'|'.$row['unit_size'].'|'.$row['product_barcode'].'|'.$row['unit_type'];					
			}
		...
		function crm_opportunity_by_revenue_list($item) {
			...
			$where = $this->crm_session_where($where);
			$q = "SELECT user_code as owner, sum(amount) as actual_revenue, (select sum(count) from crm_user_planning where owner=crm_balance.user_code and _date between '$start_date' and '$end_date' ) as target_revenue from crm_balance ";			
			
			$where .= " and deposit='sales' and _date between '$start_date' and '$end_date' and user_code<>'-' group by user_code";
			$result = $this->query_result($q, $where);
			$json = "";
			$count = 0;
			...
			while ($row = mysql_fetch_array($result)) {				
				...
				$owner = $row['owner'];
				$owner = str_replace("@battrade", "", $owner);
				$ab = 0;
				if (floatval($tab) > 0)
					$ab = $row['actual_revenue']*100/$tab;
				$json .= '{"owner":"'.$owner.'","actual_revenue":'.floatval($row['actual_revenue']).',"target_revenue":'.floatval($row['target_revenue']).',"perform":'.($ab).'},';
				$count++;
			}
		...
		function crm_chart($item) {
			...
			if ($func == "crm_product_time_expired_list") 
				return $this->crm_product_time_expired_list($item);
		...
		function crm_report($item) {
			...
			if ($func == "crm_report_sales_precent_list")
				return $this->crm_report_sales_precent_list($item);
			else
		...
		function crm_create_user_planning($item) {
			...
			for ($i = 0; $i < sizeof($owner_list); $i++) {
				$owner = $owner_list[$i];
				$q = "select owner,'price1' as pricetag from crm_users where owner='$owner' and position='Борлуулагч'";
				$result = $this->query_result($q, "");
				while ($row = mysql_fetch_array($result)) {
					$owner = $row['owner'];
					$price = $row['pricetag'];
		...
		*/
		...
		function crm_promotion_array($promotion_id) {
			...
			while ($row = mysql_fetch_array($result)) {
				...
				$array['p_type'] =	$row['p_type'];
				$array['promo_warehouse_id'] =	$row['promo_warehouse_id'];
				$array['promo_brand'] =	$row['promo_brand'];
//				$array[3] = 1;
			}
		...
		function crm_write_promo_balance($promotion_id, $mc, $promoDiscount, $item) {
			...
			while ($row_one = mysql_fetch_array($result)) {		//unegui baraanuudiig oruulah		
				...
				if($product_id == $this->crm_check_from_balance($subject,$product_id)){		
					...
					$sales_array = $this->crm_get_id_from_balance($subject,$product_id);
					$b_id_sales = $sales_array['sales'];
					$b_id_cash = $sales_array['cash'];
					$b_id_storage = $sales_array['storage'];
					//echo $b_id_sales.'  '.$b_id_cash.'  '.$b_id_storage;
					$query_two = "UPDATE crm_balance SET incoming=incoming+".$count." WHERE descr=$subject and product_id=$product_id and price=0 and id=$b_id_sales";
					mysql_query($query_two, $this->db->conn());		
					$query_two = "UPDATE crm_balance SET incoming=incoming+".$count." WHERE descr=$subject and product_id=$product_id and price=0 and id=$b_id_cash";
					mysql_query($query_two, $this->db->conn());		
					$query_two = "UPDATE crm_balance SET outgoing=outgoing+".$count." WHERE descr=$subject and product_id=$product_id and deposit='storage' and price=0 and type=0 and id=$b_id_storage";
					mysql_query($query_two, $this->db->conn());		
				}else{
					$qty = $mc * $row_one['qty'];
					$pty = $mc * $row_one['pty'];
					...
					$v = "qty=$qty&pty=$pty&type=$type&price=$price&product_id=$product_id&crm_id=$crm_id&warehouse_id=$warehouse_id&owner=$owner&amount=$amount&descr=$subject&_date=$_date&user_code=$userCode";
					$fin = new item();
					$fin->put("table", "sales");
					$fin->put("values", $v);
		...
		function crm_get_unit_size_by_one_product($product_id) {
			$query = "SELECT unit_size FROM crm_products WHERE product_id=$product_id limit 0,1";
			$result = mysql_query($query, $this->db->conn());
			while ($row_one = mysql_fetch_array($result)) {		
				$value = $row_one['unit_size'];
			}
			
			return $value;			
		}

		function crm_get_id_from_balance($descr,$product_id) {
			$array = array();
			$query = "SELECT deposit,id FROM crm_balance WHERE descr=$descr and product_id=$product_id and price=0 and type=0 ";
			$result = mysql_query($query, $this->db->conn());
			while ($row_one = mysql_fetch_array($result)) {		//unegui baraanuudiig oruulah		
//				$value = $row_one['id'];
				$array[0] = array();
				$array[$row_one['deposit']] = $row_one['id'];
			}
		...
		function crm_service_products_to_sales_action($item) {
			...
			$query33 = "select * from crm_services where service_id=$service_id and flag>0 limit 0,1";
			//$query33 = "select * from (select * from crm_services where ((service_id=$service_id and flag>0) or ((service_stage='service' or service_stage='closed') and service_id=$service_id and flag>0)) and service_qty<=0 and length(userCode)>0 and DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 5 day)<_date ) as a union all select * from ( select * from crm_services where (((service_stage='service' or service_stage='closed') and flag>0)) and service_qty<=0 and length(userCode)>0 and DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 5 day)<_date order by service_revenue desc) as b";
			$result33 = mysql_query($query33, $this->db->conn());
			while ($row33 = mysql_fetch_array($result33)) {
				$crm_id = $row33['crm_id'];
				$owner = $row33['owner'];
				$service_id = $row33['service_id'];
				...
				$subject = $row33['subject'];
				$_date = $row33['_date'];
				$userCode = $row33['userCode'];
				$warehouse_id = $row33['warehouse_id'];
				$service_precent = $row33['service_precent']; //niluuleltees bodogddog uramshuulaliin huvid huvi orj irne
				$service_revenue = $row33['service_revenue'];
				
				$count_qr = $this->num_rows("select promotion_id from crm_promotion_customers where crm_id=$crm_id and promotion_id in (select id from crm_promotions where CURRENT_TIMESTAMP between start_date and end_date)", "");
				if($count_qr > 0){

					$query21 = "select * from (select promotion_id, (select summaryTotal from crm_promotions where id=crm_promotion_customers.promotion_id) as sum,(select sum(qty) from crm_promotion_products where crm_promotion_products .promotion_id=crm_promotion_customers.promotion_id)  as qty from crm_promotion_customers where crm_id=$crm_id and (select promo_type from crm_promotions where id=crm_promotion_customers.promotion_id)='ширхэгээр' and promotion_id in (select id from crm_promotions where CURRENT_TIMESTAMP between start_date and end_date) group by promotion_id  order by (select rank from crm_promotions where crm_promotions.id=crm_promotion_customers .promotion_id) desc,(select sum(qty) from crm_promotion_products where crm_promotion_products .promotion_id=crm_promotion_customers .promotion_id) desc) as a union all select * from (select promotion_id, (select summaryTotal from crm_promotions where id=crm_promotion_customers.promotion_id) as sum,(select sum(qty) from crm_promotion_products where crm_promotion_products .promotion_id=crm_promotion_customers .promotion_id)  as qty from crm_promotion_customers where crm_id=$crm_id and (select promo_type from crm_promotions where id=crm_promotion_customers.promotion_id)='мөнгөн дүнгээр' and promotion_id in (select id from crm_promotions where CURRENT_TIMESTAMP between start_date and end_date) group by promotion_id order by (select summaryTotal from crm_promotions where id=crm_promotion_customers.promotion_id) desc) as b union all select * from (select promotion_id, (select summaryTotal from crm_promotions where id=crm_promotion_customers.promotion_id) as sum,(select sum(qty) from crm_promotion_products where crm_promotion_products .promotion_id=crm_promotion_customers .promotion_id)  as qty from crm_promotion_customers where crm_id=$crm_id and (select promo_type from crm_promotions where id=crm_promotion_customers.promotion_id)='бүлгээр' and promotion_id in (select id from crm_promotions where CURRENT_TIMESTAMP between start_date and end_date) group by promotion_id order by (select summaryTotal from crm_promotions where id=crm_promotion_customers.promotion_id) desc) as b union all select * from (select promotion_id, (select summaryTotal from crm_promotions where id=crm_promotion_customers.promotion_id) as sum,(select sum(qty) from crm_promotion_products where crm_promotion_products .promotion_id=crm_promotion_customers .promotion_id)  as qty from crm_promotion_customers where crm_id=$crm_id and (select promo_type from crm_promotions where id=crm_promotion_customers.promotion_id)='брендээр' and promotion_id in (select id from crm_promotions where CURRENT_TIMESTAMP between start_date and end_date) group by promotion_id order by (select summaryTotal from crm_promotions where id=crm_promotion_customers.promotion_id) desc) as b";
					$result21 = mysql_query($query21, $this->db->conn());
					while ($row = mysql_fetch_array($result21)) {
						$query_one = "select crm_id,service_id,type,product_id,sum(qty) as qty,sum(pty) as pty,price,precent,sum(amount) as amount,max(_date) as _date,warehouse_id from crm_deal_products_for_promotion where service_id=$service_id and flag>0 group by crm_id,service_id,type,product_id,price,precent,warehouse_id order by qty desc";
						$discount_total = 0;
						$result_one = mysql_query($query_one, $this->db->conn());
						$bool = false;
						...
						if ($promo_array['promo_type'] == 'бүлгээр') $field = 'product_type';
						if ($promo_array['promo_type'] == 'брендээр') $field = 'promo_product_brand';
						$min = 10000;
						$item = new item();
						...
						$item->put("warehouse_id", $warehouse_id);
						while ($row_one = mysql_fetch_array($result_one)) {
							...
							if($promoActive == 1 && $field == 'product_type' && $summaryTotal == 0 && $promoDiscount > 0 && $randomCount > 0 && $p_type == 'кг'){
								//if($this->crm_customer_did($crm_id, $service_id, $field) == 0){
									if($this->crm_get_product_type_count($service_id, $p_type) >= $randomCount){
										$query_two = "select id from crm_deal_products where service_id=$service_id and product_id in (select product_id from crm_products where unit_type='кг') ";
										$result_two = mysql_query($query_two, $this->db->conn());
										while ($row_two = mysql_fetch_array($result_two)) {
												$deal_id = $row_two['id'];
												$query = "update crm_deal_products set precent = $promoDiscount where id=$deal_id and precent>0";
												$result = mysql_query($query, $this->db->conn());
										}
										$this->crm_customer_insert_did($crm_id, $service_id, $field);
									}
								//}
							}

							if($promoActive == 1 && $field == 'product_type' && $summaryTotal == 0 && $promoDiscount > 0 && $randomCount > 0 && $p_type == 'ш'){
								//if($this->crm_customer_did($crm_id, $service_id, $field) == 0){
									if($this->crm_get_product_type_count($service_id, $p_type) >= $randomCount){
										$query_two = "select id from crm_deal_products where service_id=$service_id and product_id in (select product_id from crm_products where unit_type='ш') ";
										$result_two = mysql_query($query_two, $this->db->conn());
										while ($row_two = mysql_fetch_array($result_two)) {
												$deal_id = $row_two['id'];
												$query = "update crm_deal_products set precent = $promoDiscount where id=$deal_id and precent>0";
							...
							if($promoActive == 1 && $field == 'product_type' && $summaryTotal == 1 && $promoDiscount > 0 && $randomCount > 0){
								//if($this->crm_customer_did($crm_id, $service_id, $field) == 0){
									if($this->crm_get_product_type_count_multi_brand($service_id) >= $randomCount){
										$query_two = "select id from crm_deal_products where service_id=$service_id ";
										$result_two = mysql_query($query_two, $this->db->conn());
										while ($row_two = mysql_fetch_array($result_two)) {
												$deal_id = $row_two['id'];
												$query = "update crm_deal_products set precent = $promoDiscount where id=$deal_id and precent>0";
							...
							if($promoActive == 1 && $field == 'promo_product_brand' && $summaryTotal > 0 && $promoDiscount > 0 && $randomCount == 0 && $promo_brand != ''){

								//if($this->crm_customer_did($crm_id, $service_id, $field) == 0){
									//echo $this->crm_get_sum_of_selected_brand($service_id, $promo_brand).'  '.$summaryTotal;
									if($this->crm_get_sum_of_selected_brand($service_id, $promo_brand) >= $summaryTotal){
										$query_two = "select id from crm_deal_products where service_id=$service_id and product_id in (select product_id from crm_products where product_brand='$promo_brand')";
										$result_two = mysql_query($query_two, $this->db->conn());
										while ($row_two = mysql_fetch_array($result_two)) {
												$deal_id = $row_two['id'];
												$query = "update crm_deal_products set precent = $promoDiscount where id=$deal_id";
												$result = mysql_query($query, $this->db->conn());
										}

										$query_four = "delete from crm_deal_products_for_promotion where product_id in (select product_id from crm_products where product_brand='$promo_brand') and service_id=$service_id";
										mysql_query($query_four, $this->db->conn());
										$this->crm_customer_insert_did($crm_id, $service_id, $field);
									}
								//}
							}
	
							if($promoActive == 1 && $field != 'amount' && $summaryTotal == 2 && $randomCount > 0 ){
//								echo ' prom_id '.$promo_id.' deal '.$row_one[$field].' promo dtr '.$promo_array[$product_id][$field];
					//			echo ' orooood irseeeen 10+1 prcent tegleh ';
									...
									//if($this->crm_customer_did($crm_id, $service_id, $field) == 0){
										if($randomCount == 1){
											if(doubleval($this->crm_get_count_product_promotion($deal_p_id,$service_id)) >= doubleval($promo_array[$product_id][$field])){
												...
												$query_two = "update crm_deal_products_for_promotion set qty = $diff_count where service_id=$service_id and product_id=$deal_p_id";
												mysql_query($query_two, $this->db->conn());	

												$query_four = "delete from crm_deal_products_for_promotion where product_id=$deal_p_id and service_id=$service_id";
												mysql_query($query_four, $this->db->conn());	

												$query_three = "select id from crm_deal_products where service_id=$service_id and product_id=$deal_p_id";
												$result_three = mysql_query($query_three, $this->db->conn());
												while ($row_three = mysql_fetch_array($result_three)) {
														$deal_id = $row_three['id'];
														$query12 = "update crm_deal_products set precent = $promoDiscount where id=$deal_id";
														$result = mysql_query($query12, $this->db->conn());
												}
												$this->crm_customer_insert_did($crm_id, $service_id, $field);
											}
										}else{
							...
							//echo ' p_active '.$promoActive.' field '.$field.' sum total '.$summaryTotal.' p_discount '.$promoDiscount.' random '.$randomCount;

							if($promoActive == 1 && $field != 'amount' && $summaryTotal == 0 && $randomCount > 0 ){
//								echo ' prom_id '.$promo_id.' deal '.$row_one[$field].' promo dtr '.$promo_array[$product_id][$field];
								if($promo_array[$product_id][$field] > 0){
									$deal_p_id = $row_one['product_id'];
									//echo doubleval($this->crm_get_count_product_promotion($deal_p_id,$service_id));
				...
				$query_four = "select id from crm_deal_products where service_id=$service_id and flag>0";
				$result_four = mysql_query($query_four, $this->db->conn());
				while ($row_four = mysql_fetch_array($result_four)) {
						$deal_id = $row_four['id'];
						$query_two = "update crm_deal_products set flag=0 where id=$deal_id";
						$result = mysql_query($query_two, $this->db->conn());
				}
				...
				if ($bool) {
					if ($discount_total > 0) {
						...
						$warehouse_id = 0;
						$type = 0;					
						$v = "qty=$qty&pty=$pty&type=$type&price=$price&product_id=$product_id&crm_id=$crm_id&warehouse_id=$warehouse_id&owner=$owner&amount=$amount&descr=$subject&_date=$_date&user_code=$userCode&flag=$b_flag";
						$fin = new item();
					...
					mysql_query($query_three, $this->db->conn());
					$logs .= $service_id." end ".microtime(true)."\n\n";
				}
			}									
			return "success";
		}

		function crm_get_product_type_count($service_id, $p_type) {
			$count = $this->num_rows("SELECT (select product_type from crm_products where product_id=d.product_id) as ptype FROM crm_deal_products_for_promotion as d WHERE service_id=$service_id and d.product_id in (select product_id from crm_products where unit_type='$p_type') GROUP BY (select product_type from crm_products where product_id=d.product_id)", "");
			return $count;			
		}

		function crm_customer_did($crm_id, $service_id, $type) {
			$value = 0;
			$query = "SELECT crm_id FROM crm_promo_for_customer WHERE service_id=$service_id and crm_id=$crm_id and type<>'$type'";
			$result = mysql_query($query, $this->db->conn());
			while ($row_one = mysql_fetch_array($result)) {		
				$value = $row_one['crm_id'];
			}	
			return $value;			
		}

		function crm_customer_insert_did($crm_id, $service_id, $type) {
			$query_two = "INSERT INTO crm_promo_for_customer (crm_id, service_id, type) VALUES ($crm_id, $service_id, '$type')";
			$result_two = mysql_query($query_two, $this->db->conn());
		}

		function crm_get_product_type_count_multi_brand($service_id) {
			$count = $this->num_rows("SELECT (select product_type from crm_products where product_id=d.product_id) as ptype FROM crm_deal_products_for_promotion as d WHERE service_id=$service_id and d.product_id in (select product_id from crm_products where price10=1) GROUP BY (select product_type from crm_products where product_id=d.product_id)", "");
			return $count;			
		}

		function crm_get_sum_of_selected_brand($service_id, $promo_brand) {
			$value = 0;
			$query = "SELECT sum(amount) as amount FROM crm_deal_products_for_promotion as d WHERE service_id=$service_id and d.product_id in (select product_id from crm_products where product_brand='$promo_brand')";
			$result = mysql_query($query, $this->db->conn());
			while ($row_one = mysql_fetch_array($result)) {		
				$value = $row_one['amount'];
			}
			
			return $value;		
		}

		function crm_get_product_pty_byid($service_id,$product_id) {
			$value = 0;
			$query = "SELECT SUM(pty) as pty FROM crm_deal_products_for_promotion as d WHERE service_id=$service_id and d.product_id=$product_id and d.product_id in (select product_id from crm_products where price9=2)";
			$result = mysql_query($query, $this->db->conn());
			while ($row_one = mysql_fetch_array($result)) {		
				$value = $row_one['pty'];
			}
		...
		function crm_service_products_to_back_action_by_one_crm($item) {
			$logged = $_SESSION['logged']['owner'];
			$userCode = $item->get("where");
			if (!isset($userCode) || strlen($userCode) == 0) return "";
			$query1 = "select service_id from crm_services where service_stage='return' and userCode='$userCode' and flag>0 and _date>SUBDATE(now(), INTERVAL 3 day)";
			$result1 = mysql_query($query1, $this->db->conn());
			while ($row1 = mysql_fetch_array($result1)) {
...
//				$v = base64_encode($v);
		...
		function crm_action($item) {
			...
			if ($action == "insert") {
				...
				for ($d = 0; $d < sizeof($dols); $d++) {
					...
					if ($table == "crm_customer") {
						$owner = $item->get("owner");
						$query = "INSERT INTO crm_promotion_customers (crm_id,owner,promotion_id) select (select max(crm_id)+1 from crm_customer),'$owner',id from crm_promotions where CURRENT_TIMESTAMP between start_date and end_date group by id";
						mysql_query($query, $this->db->conn());
						if ($score < 20) $score = 0;
						$fls .= "mayDuplicate";
						$vls .= $score;
			...
			if ($action == "update_services_date") {
				$logged = $_SESSION['logged']['owner'];
				$table = $item->get("table");
				$values = $item->get("values");
				$where = $item->get("where");
				...
				for ($i = 0; $i < sizeof($qs); $i++) {
					if ($this->notNull($qs[$i])) {
						...
						$query = "UPDATE ".$table." SET _date='$_date' WHERE service_id=".$qs[$i];
						$result = mysql_query($query, $this->db->conn());					
						$query = "UPDATE crm_deal_products SET _date='$_date' WHERE service_id=".$qs[$i];
						$result = mysql_query($query, $this->db->conn());					
					}
				}
			...
			if ($action == "update_services_next_stage") {
				...
				for ($i = 0; $i < sizeof($qs); $i++) {
					if ($this->notNull($qs[$i])) {
						...
						$query = "UPDATE ".$table." SET owner='$owner',service_stage='$stage',remind_date=DATE(CURRENT_TIMESTAMP) WHERE service_id=".$qs[$i]." and service_stage='receipt'";
						$result = mysql_query($query, $this->db->conn());
			...
			if ($action == "update_services_undo") {
				...
				for ($i = 0; $i < sizeof($qs); $i++) {
					...
					if ($this->notNull($qs[$i])) {
						...
						$query = "UPDATE ".$table." SET service_stage='$stage',flag=1 WHERE service_id=".$qs[$i];
						$result = mysql_query($query, $this->db->conn());
						
						$query = "UPDATE crm_deal_products SET flag=1 WHERE service_id=".$qs[$i];
						$result = mysql_query($query, $this->db->conn());

						$query = "DELETE FROM crm_promo_for_customer WHERE service_id=".$qs[$i];
						$result = mysql_query($query, $this->db->conn());


						$query = "UPDATE crm_deal_products SET precent=(select discount from crm_products where product_id=crm_deal_products.product_id) WHERE service_id=".$qs[$i];
						$result = mysql_query($query, $this->db->conn());
						...
						while ($row = mysql_fetch_array($result)) {
							...
							if (strlen($descr) > 7 && intval($crm_id) > 0 && strlen($owner) > 6 && strlen($userCode)>6) {
								$query = "DELETE FROM crm_balance WHERE descr='$descr' and crm_id=$crm_id and owner='$owner' and user_code='$userCode'";
								mysql_query($query, $this->db->conn());

								$query = "DELETE FROM crm_balance_check WHERE descr='$descr' and crm_id=$crm_id and owner='$owner' and user_code='$userCode'";
								mysql_query($query, $this->db->conn());
							}
						}
					}
			...
			if ($action == "delete_all_promotion_customer") {
				$table = $item->get("table");
				$where = $item->get("where");
				if ($this->notNull($where)) {					
					$query = "DELETE FROM crm_promotion_customers WHERE promotion_id=$where";
					$result = mysql_query($query, $this->db->conn());
			...
			if ($action == "insert_promotion_customers") {
				...
				$values = $item->get("values");
				$where = $item->get("where");					
				$query = "INSERT INTO crm_promotion_customers (crm_id,owner,promotion_id) (select crm_id,owner,'$where' from crm_customer ".$_SESSION['AccountWhere'].")";
				mysql_query($query, $this->db->conn());					
				return "success";				
			} else
			...
			if ($action == "delete") {
				...
				for ($i = 0; $i < sizeof($wh); $i++) {
					...
					if ($table == 'crm_warehouse') {
						$query = "DELETE FROM ".$table." WHERE warehouse_id=".$where;
						$result = mysql_query($query, $this->db->conn());
					}
					else {
						$query = "DELETE FROM ".$table." WHERE id=".$where;
		...
		function any_delete($table, $field, $id) {
			if ($this->notNull($id) && strlen($field) > 0) {
				$logged = $_SESSION['logged']['owner'];
				$query = "DELETE FROM ".$table." WHERE $field='".$id."' and userCode='$logged' and $field<>0";
				$result = mysql_query($query, $this->db->conn());
			}
		}
		...
		function backup_tables($tables = '*') {			
			...
			{
				if ($table == "crm_gps" || $table == "crm_customer_company") continue;
		...
		function crm_login_request($item) {
			...
			if ($donation) {
				...
				if (isset($owner) && strlen($owner) > 0) {
					...
					$query = "SELECT owner,password,user_level,fullName,section,position,team,company,user_type,user_level,gmailAccount,permission,(select warehouse_id from crm_warehouse where owner='$owner' and warehouse_type='owner' limit 0,1) as warehouse_id,msg FROM crm_users WHERE user_level>=0 and length(owner)>0 and length(password)>0 and owner='$owner' and password='$password'";
					$result = mysql_query($query, $this->db->conn());
					while ($row = mysql_fetch_array($result)) {
						$_SESSION['logged'] = $row;				
...
//						return "logged,".$row['user_type'].$row['msg'];
						return "logged,".$row['user_type'].",".$row['phone'].",".$row['msg']." ,2.9,".$row['warehouse_id'];
					}
...
//				mysql_query("INSERT INTO crm_login_attempts(owner, _date) VALUES ('$user', '$now')", $this->db->conn());
				$query = "SELECT owner,password,user_level,fullName,section,position,team,company,user_type,user_level,gmailAccount,permission,(select warehouse_id from crm_warehouse where owner='$user' and warehouse_type='owner' limit 0,1) as warehouse_id,msg FROM crm_users WHERE user_level>=0 and length(owner)>0 and length(password)>0 and owner='$user' and password='$password'";
				$result = mysql_query($query, $this->db->conn());
				while ($row = mysql_fetch_array($result)) {
					$_SESSION['logged'] = $row;
//					if ($row['user_level'] >= 3)
...
//					return "logged,".$row['user_type'].$row['msg'];
		...
		function crm_xls_json($filepath, $name) {
			...
			try {
				...
				if ($name == "Product") {
					$basic_fields = "product_code,product_barcode,product_type,product_name,product_brand,product_vendor,unit_type,unit_size,price,company,price1,price2,price3,price4,price5,price6,price7,price8,price9,price10,warehouse_id,unit_metric,userCode";
					$bf = explode(",", $basic_fields);
					$table = "crm_products";
				} else
				if ($name == "Deal") {
				...
				if ($name == "Storage") {
					...
					foreach ($Spreadsheet as $Key => $Row) {
						...
						if ($Row && $i >= 10) {
							if (doubleval($Row[4]) == 0 || intval($Row[7]) == 0) continue;
		...
		function crm_export_xls($name) {			
			...
			if ($name == 'Storage')
				$query = "SELECT * FROM crm_storage_list";
			if ($name == 'CRM_REPORT_STORAGE')
				$query = "select * from crm_report_storage";
			if ($name == 'CRM_REPORT_SALES_PRECENT')
				$query = "select service_id,(select firstname from crm_customer where crm_id=crm_deal_products_for_sales_report.crm_id) as customer_name,(select product_name from crm_products where product_id=crm_deal_products_for_sales_report.product_id)as product_name,(select product_code from crm_products where product_id=crm_deal_products_for_sales_report.product_id)as product_code,precent,pty,qty,price,amount,type,crm_id,warehouse_id,_date,descr,userCode,unit_size,flag from crm_deal_products_for_sales_report";
	...
	class crm_fact {
		...
		function filegw($item) {
			...
			if ($item->get("action") == "image_upload") {				
...
//				if ($uploaded_type == 'image/gif') {
					if(move_uploaded_file($_FILES['file-path']['tmp_name'], $target_path)) {	
						$response = array('success'=>true, 'status'=>'ok', 'msg'=> 'sucess');
						return json_encode($response);
					} else
						return '{"success": "false", "msg":"Алдаа гарлаа дахин оролдоно уу!"}';
//				} else
//						return '{"success": "false", "msg":"Зургийн төрөл буруу байна GIF төрөлтэй байх ёстой !"}';
			} else

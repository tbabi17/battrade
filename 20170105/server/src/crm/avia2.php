<?
	...
	class crm_storage extends crm_task{
		var $basic_fields = "warehouse_id,product_id,product_code,warehouse_name,product_name,aty,qty,pty,_date";
	}
	
	class crm_product extends crm_task{
		var $basic_fields = "product_id,product_name,product_type,product_code,product_barcode,product_brand,product_vendor,price,unit_type,unit_size,company,price1,price2,price3,price4,price5,price6,price7,price8,price9,price10,warehouse_id,unit_metric,discount";
	}
	...
	class crm_user extends crm_task{
		var $basic_fields = "id,owner,password,fullName,phone,section,position,team,manager,gmailAccount,user_type,company,user_level,permission,partner,warehouse_id,mon,thue,wed,thur,fri,sat,sun";
	}
	...
	class crm {		
		...
		function crm_is_non_like($fls) {
			if ($fls == "id" || $fls == "_date" || $fls == "crm_id" || $fls == "mayDuplicate" || $fls == "parent_crm_id" || $fls == "customer_type" || $fls == "birthday" || $fls == "work_type" || $fls == "times" || $fls == "status" || $fls == "notify" || $fls == "deal_id" || $fls == "case_id" || $fls == "probablity" || $fls == "expected_revenue" || $fls == "actual_revenue" || $fls == "closing_date" || $fls == "qty" || $fls == "pty" || $fls == "total" || $fls == "partner" || $fls == "aty") 
				return true;
		...
		function crm_product_available_xml_list($item) {
			...
			$result = $this->query_result($q, " WHERE total>0 and length(product_type)>0 ORDER BY $sort $dir");
			while ($row = mysql_fetch_array($result)) {
...
//			$where = $this->crm_session_userCode_where($where);
		...
		function crm_storage_list($item) {
			...
			$q = "SELECT warehouse_id,product_id,(select total from crm_products where product_id=b.product_id) as aty, sum(incoming)-sum(outgoing) as qty,sum(pty) as pty,max(_date) as _date FROM crm_balance as b";
			...
			$result = $this->query_result($q, $where." and deposit='storage' and warehouse_id=1 and _date>='2014-05-20' GROUP BY product_id ORDER BY $sort $dir LIMIT $start,$end");
		...
		function crm_report_product_list($item) {
			...
			$where = $this->crm_session_where($where);
			$count = $this->num_rows($q, $where);
			$result = $this->query_result($q, $where." and deposit='sales' group by product_id ORDER BY product_id ASC");
			$_SESSION['CRM_REPORT_PRODUCT'] = $q." ".$where." and deposit='sales' group by product_id ORDER BY product_id ASC";
		...
		function crm_report_storage_daily_list($item) {
			...
			$_SESSION['CRM_REPORT_STORAGE_DAILY'] = $q." ".$where." and product_id > 0 and warehouse_id<3 group by product_id  ORDER BY warehouse_id,product_id ASC";
			while ($row = mysql_fetch_array($result)) {
...
//			$where = $this->crm_where($item, $record->basic_fields);
//			$where = $this->crm_session_where($where);
			$count = $this->num_rows($q, $where." WHERE position='Борлуулагч' and (_date>=date(CURRENT_TIMESTAMP) or owner in (select owner from crm_gps where _date>=date(CURRENT_TIMESTAMP) group by owner))");
		...
		function crm_service_products_to_sales_action($item) {
			...
			while ($row = mysql_fetch_array($result)) {
				...
				while ($row_one = mysql_fetch_array($result_one)) {
					...
					$user_code = $row_one['userCode'];
					$warehouse_id = $row_one['warehouse_id'];
					$_date = $row_one['_date'];
					$type = ($row_one['type'] == 'cash' ? 0 : 1);					
					$v = "qty=$qty&pty=$pty&type=$type&price=$price&product_id=$product_id&crm_id=$crm_id&warehouse_id=$warehouse_id&owner=$owner&amount=$amount&descr=$subject&_date=$_date&user_code=$user_code";
					$fin = new item();
					$fin->put("table", "sales");
					$fin->put("values", $v);
			...
			if ($item->get("action") == "image_upload") {				
				...
				if ($uploaded_type == 'image/png' || $uploaded_type == 'image/jpeg' || $uploaded_type == 'image/gif') {
					if(move_uploaded_file($_FILES['file-path']['tmp_name'], $target_path)) {	
						...
						return '{"success": "false", "msg":"Зургийн төрөл буруу байна !"}';
			} else

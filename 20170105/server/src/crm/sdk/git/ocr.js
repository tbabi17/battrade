		...
		me.actions = [
					...
					if (service_stage == 'service') {
						if (userCode == 'amarjargal@cosmo' || userCode == 'amarjargalgoo@cosmo'  || userCode == 'bayarchimeggoo@cosmo' || userCode == 'bayarchimeg@cosmo' || userCode == 'otgonbat@cosmo' || userCode == 'narantsetseg@cosmo')												
							...
							if (logged.indexOf('battrade') != -1)
								window.open('http://'+ip+'/invms/?values='+userCode+';'+crm_id+';'+date+';1;4;'+logged+';'+subject,'');
							else
			...
			Ext.create('Ext.Action', {
				...
				handler: function(widget, event) {
					...
					date = me.grid.getView().getSelectionModel().getSelection()[0].get('_date').split(' ')[0]; 
						...
						window.open('http://'+ip+'/invms32/?values='+userCode+';'+crm_id+';'+date+';1;4;'+logged+';'+subject+';1','');				
				}
			}),
			Ext.create('Ext.Action', {
				iconCls   : 'calendar',
				...
				hidden: !(logged.indexOf('battrade') != -1),
				handler: function(widget, event) {
					var count = (me.selectedIds('service_stage').match(/receipt:/g) || []).length;
					console.log(count+" same words");
					if(count>1){
						var splits = me.selectedIds('service_stage').replace(/:$/, '').split(':');
						
						console.log(splits);
						if (user_level > 0 ) {
							if (me.recordSelected())						
								new OCS.ServiceDateChangeWindow({
									selected: me.grid.getView().getSelectionModel().getSelection(),
									ids: me.selectedIds('service_id'),
						...
						} else 
						{
							...
							console.log(me.selectedIds('service_id'));
							console.log(me.selectedIds('service_stage'));
						}
					}
			...
			Ext.create('Ext.Action', {
				...
				handler: function(widget, event) {
					...
					window.open('http://'+ip+'/defrag/?values='+userCode+';'+crm_id+';'+date+';1;1;7',''); 					
				}
			}),
			Ext.create('Ext.Action', {
				iconCls   : 'deal_undo',
		...
		me.actions = [		
			...
			'-',
			{
				  ...
				  store: Ext.create('Ext.data.Store', {
					  model: 'CRM_ITEM',
					  data: [{value: 'ирсэн'},{value: 'зөвшөөрсөн'}]
				  }),

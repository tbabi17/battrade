			...
			listeners: {
				load : function(store, records, successful, operation, eOpts) {
						...
						if (me.func == 'crm_alarm_list' && me.store.getCount() > 0) {
							var deal = 0, ccase = 0, activity = 0, cservice = 0, tproduct = 0;
			...
			if(btn === 'yes'){
				if (selection.length == 1) {
					if (user_level < 2 && selection[0].get('owner') && selection[0].get('owner') != logged && selection[0].get('userCode') != logged) {
						Ext.MessageBox.alert('Status', 'Уг үйлдлийг хийхэд таны эрх хүрэлцэхгүй 111!', function() {});
						return;
					}
...
//					me.form.setVisible(true);
				...
				views['storage'].storageReload(selections[0]);
			},			
			rowselect: function(sm, rowIdx, r) {
				me.form.updateSource(selections[0]);					
			}
...
Ext.define('OCS.ProductGridWithFormPanel', {	
	...
	createActions: function(actions) {
		...
		me.actions = [
			...
			Ext.create('Ext.Action', {
				iconCls   : 'add',
				text: 'Хугацаа оруулах...',
				handler: function(widget, event) {
					if (me.recordSelected())
						new OCS.TimeoutWindow({
							product_id: me.selectedIds('product_id'),
							selected: me.grid.getView().getSelectionModel().getSelection()[0]
						}).show();
				}
			}),
			Ext.create('Ext.Action', {
				iconCls   : 'add',
				text: 'Олноор нь засах...',
				handler: function(widget, event) {
					if (me.recordSelected())
						new OCS.MultiEditWindow({
							product_id: me.selectedIds('product_id'),
							selected: me.grid.getView().getSelectionModel().getSelection()[0]
						}).show();
				}
...
Ext.define('OCS.MultiEditWindow', {
	extend: 'OCS.Window',
	title: 'Засах',
	table: 'crm_products',
	width: 350,
	...
	initComponent: function() {
		...
		me.form = Ext.create('OCS.FormPanel', {
			...
			},
			{
				...
				model: 'CRM_ITEM',
					 data: [{value: 'product_brand'},{value: 'company'},{value: 'unit_type'},{value: 'product_type'},{value: 'discount'}]
				}),
				name: 'source_value',
				queryMode: 'local',
				displayField: 'value',
			...
			buttons: [{
				...
				handler: function() {
					var form = this.up('form').getForm();
					if(form.isValid()){
						var col = form.findField('source_value').getValue();
						var product_ids = me.product_id.split(':');
						if(col != '' && form.findField('value').getValue() != ''){
							for (i = 0; i < product_ids.length; i++) {
								var product_id = product_ids[i];
									...
									Ext.Ajax.request({
									   ...
									   params: {handle: 'web', table: 'crm_products', action: 'update', values: col+"='"+form.findField('value').getValue()+"'", where: 'product_id='+product_id},
									   success: function(response, opts) {							  							  
...
Ext.define('OCS.TimeoutWindow', {
	extend: 'OCS.Window',
	title: 'Enter time expire',
	table: 'crm_products_time_expired',
	width: 350,
	height: 300,
	...
	initComponent: function() {
		...
		me.form = Ext.create('OCS.FormPanel', {
			...
			},
			{
				  ...
				  store: Ext.create('Ext.data.Store', {
					  model: 'CRM_OBJECT',
					  data: [{id:1, value: 'Төв агуулах'},{id: 4, value: 'Жижиглэн агуулах'}]
				  }),
				  name: 'warehouse_id',
				  queryMode: 'local',
				  displayField: 'value',
	...
	getEditor: function(name, value) {
		...
		if (name == 'promo_type')
		{
			return {
			  ...
			  store: Ext.create('Ext.data.Store', {
  				  model: 'CRM_ITEM',
 				  data: [{value: 'ширхэгээр'},{value: 'хайрцагаар'},{value: 'мөнгөн дүнгээр'},{value: 'бүлгээр'},{value: 'брендээр'}]
              }),
			  name: name,
			  queryMode: 'local',
		      displayField: 'value',
		...
		if (name == 'unit_type')
		{
			return {
			  ...
			  store: Ext.create('Ext.data.Store', {
				 model: 'CRM_ITEM',
				 data: [{value: 'ш'},{value: 'кг'},{value: 'л'},{value: 'хр'},{value: 'блок'}]
			  }),
			  name: name,
			  queryMode: 'local',
		      displayField: 'value',

		...
		me.form = Ext.create('OCS.FormPanel', {
			...
			buttons: [{
				...
				handler: function() {
					...
					if (form.isValid())	{
						var values = form.getValues(true);
						var _date = Ext.Date.format(form.findField('_date').getValue(),'Y-m-d H:i:s');
	...
	initComponent: function() {
		...
		me.form = Ext.create('OCS.FormPanel', {
			...
			items: [{
				...
				fieldLabel: 'Source',
				hidden: true,
				value: ( me.selected.get('warehouse_type') == 'owner' ?1:3),
	...
	addProduct: function(form) {
		...
		*/
		if (form.findField('qty').getValue() != 0) {								
			...
			values = "qty=-"+form.findField('qty').getValue()+"&type=0&pty="+pty+"&price=0&product_id="+form.findField('product_id').getValue()+"&crm_id=0&warehouse_id="+form.findField('source_warehouse_id').getValue()+"&owner="+logged+"&amount=0&descr="+descr;
			Ext.Ajax.request({
			   url: 'avia.php',
			   params: {handle: 'web', func: 'crm_finance_fun', table: 'storage', action: 'balance', values: values, where: ''},
			   success: function(response, opts) {							  
